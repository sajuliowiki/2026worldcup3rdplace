// combo-matrix.js - All 495 third-place advancement combinations with R32 matchups
// Format: [groups_string, [1A_opp, 1B_opp, 1D_opp, 1E_opp, 1G_opp, 1I_opp, 1K_opp, 1L_opp]]

const COMBO_MATRIX_RAW = [
...
const COMBO_MATRIX = COMBO_MATRIX_RAW.map(row => {
  const groups = row[0];
  return {
    groups: groups,
    groupSet: new Set(groups.split('')),
    matchups: {
      "1A": row[1], "1B": row[2], "1D": row[3], "1E": row[4],
      "1G": row[5], "1I": row[6], "1K": row[7], "1L": row[8]
    }
  };
});
